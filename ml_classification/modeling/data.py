...
from ml_classification.config import S3_BUCKET, REPORTS_DIR
import typer
import boto3
...
def main() -> pd.DataFrame:
    ...
    get_table_metadata("/gold/credit_card_default_features.parquet")
    target_col = "default_payment_next_month"
    logger.info(f"Loading dataset from: {data_path}")
    df = pd.read_parquet(data_path, storage_options={"anon": False})  # assumes AWS credentials are configured
